...
// Defaults per requirements: lite=75, pro=300, elite=1500
const LITE_CREDITS = Number(process.env.LITE_CREDITS || 75);
const PRO_CREDITS = Number(process.env.PRO_CREDITS || 300);
const ELITE_CREDITS = Number(process.env.ELITE_CREDITS || 1500);
...
const PRODUCT_MAP: Record<"lite" | "pro" | "elite", Set<string>> = {
  lite: new Set(
    (process.env.RC_PRODUCTS_LITE || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
  ),
  pro: new Set(
    (process.env.RC_PRODUCTS_PRO || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
  ),
  elite: new Set(
    (process.env.RC_PRODUCTS_ELITE || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
  ),
...
function detectTier(payload: any): "lite" | "pro" | "elite" | null {
  ...
  const entitlementIdPrimary: string =
    ...
    payload?.entitlement_identifier ||
    ev?.entitlement_id ||
    ev?.entitlement_identifier ||
    "";

  const entitlementIdsArray: string[] = Array.isArray(payload?.entitlement_ids)
    ? payload.entitlement_ids
    : Array.isArray(ev?.entitlement_ids)
    ? ev.entitlement_ids
    : [];

  const entitlementIds = entitlementIdPrimary
    ? [entitlementIdPrimary, ...entitlementIdsArray]
    : entitlementIdsArray;
  ...
  for (const tier of ["lite", "pro", "elite"] as const) {
    ...
    for (const e of entitlementIds)
      if (ENTITLEMENT_MAP[tier].has(e)) return tier;
  }
  // 2) Fallback to substring heuristic
  const all = `${productId} ${entitlementIdPrimary} ${entitlementIds.join(
    " "
  )}`.toLowerCase();
  ...
  try {
    ...
    if (eventId) {
      ...
      if (exists) {
        return res
          .status(200)
          .json({ ok: true, skipped: true, reason: "duplicate_event" });
      }
    }
